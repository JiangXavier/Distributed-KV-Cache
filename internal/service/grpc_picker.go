...
func (s *Server) SetPeers(peersAddrs []string) {
	...
	for _, peersAddr := range peersAddrs {
		...
		s.clients[peersAddr] = NewClient("LeiCache")
	}
	s.mu.Unlock()
...
func (s *Server) reconstruct() {
	serviceList, err := discovery3.ListServicePeers("LeiCache")
	...
	for _, peerAddr := range serviceList {
		...
		s.clients[peerAddr] = NewClient("LeiCache")
	}
	s.mu.Unlock()
...
func (s *Server) Start() {
	...
	go func() {
		...
		err := discovery3.Register("LeiCache", s.Addr, s.stopsSignal)
		if err != nil {
